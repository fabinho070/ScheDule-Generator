...
<br><br><h2>Turmas e Horário Semanal:</h2><br>

<p style="color:#fff">Escolha uma turma e selecione seu respectivo horário:</p>
<p style="color:white;"><b>ATENÇÃO!</b> Salve todos as turmas com o botão <b>ADICIONAR OUTRA TURMA</b>, e somente após salvar todas clique no botão <b>PRÓXIMO</b></p>

</div>

<form name="form1" method="POST" action="FUNCOES/6funcaoseries.php">

<div> 
     <select name="turmas" style="width:600px; height:40px; border-radius:15px;" class="btn-3d btn-light">
     <?php while ($dadosturmas = mysqli_fetch_assoc($turmas)) { ?>
     <?php foreach($dadosturmas as $mostrarturmas){ ?>
     <option class="btn-light" value="<?php echo $mostrarturmas; ?>"> TURMA: <?php echo $mostrarturmas; } ?> </option>
     <?php } ?>
     </select>
     </div>
...
<button class="btn btn-light btn-3d circulo" type="submit"><b>Adicionar Outra Turma</b></button><BR> 

<a class="btn btn-primary btn-3d circulo" href="5propostahorario.php">Voltar</a>
<a class="btn btn-success btn-3d circulo" href="7todos_os_dados.php">Próximo</a>
</form>
</div>
